...
adrs_vigentes:
  ...
  - "docs/adr/0006-arquitectura-event-driven-dominio.md"
skills_aplicados:
  - "docs/skills/c4.md"
...
# Documento Técnico Inicial del Producto (DTI) — vFinal (Defensa)
...
| Grupo | G01 |
| Versión | **vFinal** (evolución desde v1.0) |
| Fecha | 18/05/2026 |
...
### 6.0 Estado actual (v1.0 piloto) vs objetivo (v2.0)

| Dimensión | v1.0 (piloto — ADR-0001, ADR-0004) | v2.0 (objetivo — ADR-0005, ADR-0006) |
|-----------|-------------------------------------|--------------------------------------|
| Estilo | Monolito modular hexagonal | Monolito modular + **workers ECS** desacoplados |
| Mensajería | Bull + Redis in-process | **Outbox** + EventBridge + SQS |
| Despliegue | Docker Compose on-prem | **AWS** ECS Fargate + RDS + ElastiCache |
| Sagas distribuidas | **No** — RB-06 en una transacción PostgreSQL | **No** en DJ; eventualidad solo post-commit |

> **v1.0:** No hay microservicios ni sagas compensatorias entre servicios. La comunicación asíncrona (notificaciones SMTP) se implementa con Bull + Redis **dentro del monolito**.
...
> Extracción del worker y bus gestionado: ver **ADR-0005** (AWS) y **ADR-0006** (event-driven).

### 6.1 Transición C4 ↔ Hexagonal ↔ Event-Driven

```mermaid
flowchart LR
  C4[C4 N1-N2\nActores y contenedores] --> HEX[Hexagonal\ndomain ports]
  HEX --> EV[Eventos de dominio\npost-commit]
  EV --> AWS[AWS ECS + SQS\nADR-0005]
```

| Vista | Artefacto | Trazabilidad |
|-------|-----------|--------------|
| C4 Contexto | `docs/diagrams/c4_ctx_sgai.mmd` | Actores BRD §10 |
| C4 Contenedores | `docs/diagrams/c4_cont_sgai.mmd` | FSD-UC-001–010 |
| Hexagonal | §5 de este DTI | ADR-0001 |
| Event-driven | §7 + ADR-0006 | RB-06, FSD-UC-002 |
| Despliegue AWS | `docs/diagrams/deploy_aws_v2_target.mmd` | ADR-0005 |
...
### 8.5 Mapeo de componentes a AWS (Release 2.0.0) `[humano+máquina]`

> **Referencia:** ADR-0005. El piloto on-prem (§8.2) permanece vigente hasta gate de go-live CEUB; esta sección describe el **estado objetivo** para la defensa y el roadmap.

| Componente lógico (C4) | Servicio AWS | Notas |
|------------------------|--------------|-------|
| SPA React | Amazon S3 + CloudFront | Build estático; headers CSP |
| API REST (hexagonal) | ECS Fargate `sgai-api` | Health check `/health`; auto-scaling CPU |
| Worker notificaciones | ECS Fargate `sgai-worker` | Consume SQS; idempotencia por `jobId` |
| PostgreSQL | RDS PostgreSQL 15 Multi-AZ | Cifrado KMS; backups automáticos |
| Redis / cola | ElastiCache Redis 7 + **SQS** | Eventos críticos vía SQS (ADR-0006) |
| Bus de eventos | Amazon EventBridge | Reemplazo evolutivo de publicación Bull directa |
| Secretos | AWS Secrets Manager | JWT, SMTP, credenciales RDS |
| Observabilidad | CloudWatch Logs + Métricas | `correlationId` en cada log line |
| LDAP / SMTP on-prem | VPN / Direct Connect | Sin exponer directorio a internet público |

**Diagrama:** `docs/diagrams/deploy_aws_v2_target.mmd`

**Soberanía Ley 164:** región preferente `sa-east-1`; cláusulas de procesamiento en contrato institucional; PII cifrada (KMS + columnas AES en app).
...
- **Resultado:** ✅ **Pass** — ver `pocs/POC-01/log.md` (15 registros, p95 < 5 s).
...
- **Resultado:** ✅ **Pass** — ver `pocs/POC-02/log.md` (orphan=0, p95=420 ms).
...
| 0004 | Despliegue on-premise con Docker Compose en servidores universitarios | **Aceptada** (piloto v1.0) | 10/05/2026 |
| 0005 | Proveedor cloud AWS y estilo de despliegue objetivo (ECS, RDS, SQS) | **Aceptada** (v2.0) | 18/05/2026 |
| 0006 | Arquitectura event-driven — Outbox + EventBridge/SQS | **Aceptada** (v2.0) | 18/05/2026 |

> ADR completas con tablas de pros/contras: `docs/adr/NNNN-*.md`
...
- [x] Arquitectura distribuida: evolución v1→v2 documentada (§6 + ADR-0006).
- [x] Despliegue on-premise piloto (§8) + **mapeo AWS objetivo** (§8.5 + ADR-0005).
...
- [x] 6 ADRs registradas en §21 (`docs/adr/`).
- [x] Vistas 4+1: `docs/DTI_AVANCE_VISTAS_4MAS1.md`.
- [x] 2 POCs ejecutadas documentadas en `pocs/POC-01`, `pocs/POC-02` (§12).
...
| v1.1 | 17/05/2026 | Carolina Aguilar | §3.5.1 prompts/guardrails/tools; anexo vistas 4+1 |
| vFinal | 18/05/2026 | Carolina Aguilar | `docs/DTI.md`; §6 transición distribuida; §8.5 AWS; ADR-0005/0006; POCs y roadmap |
